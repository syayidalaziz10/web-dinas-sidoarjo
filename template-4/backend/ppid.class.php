...
class PPID
{
	public static function Show($dbCon, $data = ""){
		try {
			$pre = $dbCon->prepare("SELECT a.id as idnews, a.iduser , a.title, a.description , a.img, a.date, b.id, b.full_name FROM tb_news a LEFT JOIN tb_users b ON a.iduser = b.id WHERE category = 6 ORDER BY date ASC");
			$pre->execute();
